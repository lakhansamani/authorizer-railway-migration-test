FROM lakhansamani/authorizer:2.0.0-rc.3

# Base image runs as USER authorizer (uid 1000).
# For SQLite, ensure the mounted /data directory is writable by that user.
CMD ["/bin/sh", "-c", "\
  ./authorizer \
  ...
  \
  --google-client-id=\"${GOOGLE_CLIENT_ID}\"       --google-client-secret=\"${GOOGLE_CLIENT_SECRET}\"       --google-scopes=\"${GOOGLE_SCOPES}\" \
  --github-client-id=\"${GITHUB_CLIENT_ID}\"       --github-client-secret=\"${GITHUB_CLIENT_SECRET}\"       --github-scopes=\"${GITHUB_SCOPES}\" \
  --facebook-client-id=\"${FACEBOOK_CLIENT_ID}\"   --facebook-client-secret=\"${FACEBOOK_CLIENT_SECRET}\"   --facebook-scopes=\"${FACEBOOK_SCOPES}\" \
  --microsoft-client-id=\"${MICROSOFT_CLIENT_ID}\" --microsoft-client-secret=\"${MICROSOFT_CLIENT_SECRET}\" --microsoft-tenant-id=\"${MICROSOFT_TENANT_ID}\" --microsoft-scopes=\"${MICROSOFT_SCOPES}\" \
  --apple-client-id=\"${APPLE_CLIENT_ID}\"         --apple-client-secret=\"${APPLE_CLIENT_SECRET}\"         --apple-scopes=\"${APPLE_SCOPES}\" \
  --discord-client-id=\"${DISCORD_CLIENT_ID}\"     --discord-client-secret=\"${DISCORD_CLIENT_SECRET}\"     --discord-scopes=\"${DISCORD_SCOPES}\" \
  --linkedin-client-id=\"${LINKEDIN_CLIENT_ID}\"   --linkedin-client-secret=\"${LINKEDIN_CLIENT_SECRET}\"   --linkedin-scopes=\"${LINKEDIN_SCOPES}\" \
  --twitch-client-id=\"${TWITCH_CLIENT_ID}\"       --twitch-client-secret=\"${TWITCH_CLIENT_SECRET}\"       --twitch-scopes=\"${TWITCH_SCOPES}\" \
  --twitter-client-id=\"${TWITTER_CLIENT_ID}\"     --twitter-client-secret=\"${TWITTER_CLIENT_SECRET}\"     --twitter-scopes=\"${TWITTER_SCOPES}\" \
  --roblox-client-id=\"${ROBLOX_CLIENT_ID}\"       --roblox-client-secret=\"${ROBLOX_CLIENT_SECRET}\"       --roblox-scopes=\"${ROBLOX_SCOPES}\" \
  \
